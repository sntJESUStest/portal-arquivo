...
const DROPBOX_APP_SECRET = 'o2ggocygo1yxrij';

// Pasta raiz no Dropbox — ignora BACKUP/RH e começa daqui
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // continua mesmo sem verificação em dev
  }
...
async function processarAlteracoes() {
  try {
    // Listar arquivos na pasta raiz BACKUP/RH
    ...
    if (!data.entries) {
      console.error('Erro Dropbox:', data);
      return;
    }
    ...
    for (const arquivo of arquivos) {
      // Remover o prefixo /BACKUP/RH e separar partes
      // Estrutura esperada: /BACKUP/RH/Empresa/Setor/MM-YYYY/arquivo.pdf
